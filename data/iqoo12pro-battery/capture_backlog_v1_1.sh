#!/system/bin/sh
# iQOO 12 Pro Scene battery ledger - one-shot backlog drain v1.1
# One manual handoff only: start from Runner, switch once to Scene -> 耗电统计.
# After that, keep the whole drain inside ActivityPowerStat:
# History -> detail -> capture -> reopen History from the detail toolbar -> repeat.
# Do NOT use Back after detail capture; Back can leave ActivityPowerStat and return
# to ActivityMain, which is exactly what broke v1.0 after the first successful row.
...
reopen_history_from_detail() {
  ...
  while [ "$RH_TRY" -lt 6 ]; do
    ...
    # ActivityPowerStat keeps the History action in its toolbar. Prefer reopening
    # the dialog directly from the historical detail screen instead of Back.
    ...
    # If the app bar was hidden/collapsed after detail pagination, move content
    # toward the top and retry. This remains inside ActivityPowerStat.
...
echo '# iQOO 12 Pro Scene backlog drain v1.1'
...
while [ "$CAPTURED" -lt "$MAX_CAPTURES" ]; do
  ...
  if [ "$SCAN_RC" -eq 0 ]; then
    ...
    CAP_RC=$?
    [ "$CAP_RC" -eq 0 ] || {
      echo "ERROR reopen_after_capture rc=$CAP_RC session=$TARGET"
      exit "$CAP_RC"
    }
    CAPTURED=$NEXT_SEQ
    # Opening a fresh History dialog resets its viewport, so restart end-of-list
    # detection. Manifest dedupe guarantees already captured rows are skipped.
    PREV_SCROLL_SIG=''
    continue
  fi
