#!/system/bin/sh
# iQOO 12 Pro Scene battery ledger - one-shot backlog drain v1.4
#
# v1.4 hardens Scene app attribution capture against the RecyclerView/AppInfoLoader
# async-label race. Each detail viewport is allowed to settle, then its ordered
# itemTitle+bounds signature must be identical in two consecutive dumps before the
# page is accepted. A viewport that still contains only the default "Scene" label
# is never accepted on the first app page; the worker waits up to LABEL_WAIT_TRIES.
#
# Existing raw files remain immutable. Manifest semantics are append-only and the
# latest state for a session is authoritative. Previously-audited invalid sessions
# are therefore selected again automatically.

BASE=/sdcard/SceneBattery
OUTDIR="$BASE/sync_raw"
INVALIDDIR="$BASE/sync_raw_invalid"
DIAGDIR="$BASE/diagnostics"
MANIFEST="$BASE/sync_manifest.csv"
RUN_ID="$(date '+%Y%m%d-%H%M%S')-$$"
TMP="$BASE/backlog_tmp/$RUN_ID"
mkdir -p "$OUTDIR" "$INVALIDDIR" "$DIAGDIR" "$TMP"
...
LABEL_INITIAL_SETTLE=${SCENE_BATTERY_LABEL_INITIAL_SETTLE:-2}
LABEL_STABLE_DUMPS=${SCENE_BATTERY_LABEL_STABLE_DUMPS:-2}
...
raw_attribution_health() {
  ...
  : > "$RAH_TITLES"
  grep 'resource-id="com.omarea.vtools:id/itemTitle"' "$RAH_FILE" 2>/dev/null \
    | sed -n 's/.*text="\([^"]*\)".*/\1/p' | sed '/^$/d' > "$RAH_TITLES"
...
should_skip_session() {
  ...
  raw_attribution_health "$ML_FILE"; SSS_RC=$?
  ...
  while IFS= read -r SCH_LINE; do
    case "$SCH_LINE" in
      ...
      *'resource-id="com.omarea.vtools:id/ItemCenter"'*)
        ...
        TARGET="$SCH_ROW_TITLE"; TARGET_USED="$SCH_USED_H"; TARGET_Y=$(( (SCH_Y1 + SCH_Y2) / 2 ))
        return 0
        ;;
    esac
  done < "$SCH_N"
...
open_target_detail() {
  ...
  while [ "$OTD_TRY" -le "$DETAIL_TAP_RETRIES" ]; do
    ...
    if grep -q 'resource-id="com.omarea.vtools:id/avg_power"' "$OTD_VERIFY" && ! grep -q 'text="历史记录"' "$OTD_VERIFY"; then
      DETAIL_VERIFY_FILE="$OTD_VERIFY"; return 0
    fi
    [ "$OTD_TRY" -lt "$DETAIL_TAP_RETRIES" ] || return 32
    is_history "$OTD_VERIFY" || return 32
    sleep 1
    OTD_HISTORY="$TMP/detail-retry-history-$OTD_SEQ-$OTD_TRY.xml"; dump "$OTD_HISTORY" || return 30
...
# Wait until the visible app-label layout is stable. Signature includes ordered
# itemTitle text + bounds so a RecyclerView still moving/rebinding cannot pass.
# SESSION_HAS_NONSCENE is set by capture_target_detail and persists across pages.
wait_stable_detail_page() {
  WSD_SEQ="$1"; WSD_PAGE="$2"; WSD_TRY=1; WSD_PREV_SIG=''; WSD_STABLE=0
  WSD_LAST_XML=''; WSD_LAST_TITLES=''

  [ "$LABEL_INITIAL_SETTLE" -gt 0 ] && sleep "$LABEL_INITIAL_SETTLE"

  while [ "$WSD_TRY" -le "$LABEL_WAIT_TRIES" ]; do
    WSD_XML="$TMP/stable-$WSD_SEQ-$WSD_PAGE-$WSD_TRY.xml"
    WSD_N="$TMP/stable-$WSD_SEQ-$WSD_PAGE-$WSD_TRY.nodes"
    WSD_TITLES="$TMP/stable-titles-$WSD_SEQ-$WSD_PAGE-$WSD_TRY.txt"
    WSD_SIGFILE="$TMP/stable-sig-$WSD_SEQ-$WSD_PAGE-$WSD_TRY.txt"

    dump "$WSD_XML" || return 73
    grep -q 'package="com.omarea.vtools"' "$WSD_XML" || return 74
    grep -q 'text="历史记录"' "$WSD_XML" && return 75
    nodes "$WSD_XML" > "$WSD_N"

    grep 'resource-id="com.omarea.vtools:id/itemTitle"' "$WSD_N" \
      | sed -n 's/.*text="\([^"]*\)".*bounds="\([^"]*\)".*/\1|\2/p' > "$WSD_SIGFILE"
    cut -d'|' -f1 "$WSD_SIGFILE" | sed '/^$/d' > "$WSD_TITLES"

    WSD_COUNT="$(wc -l < "$WSD_TITLES" | tr -d ' ')"; [ -n "$WSD_COUNT" ] || WSD_COUNT=0
    WSD_NONSCENE="$(grep -vc '^Scene$' "$WSD_TITLES" 2>/dev/null || true)"; [ -n "$WSD_NONSCENE" ] || WSD_NONSCENE=0
    WSD_SIG="$(cksum "$WSD_SIGFILE" 2>/dev/null | awk '{print $1":"$2}')"

    if [ -n "$WSD_PREV_SIG" ] && [ "$WSD_SIG" = "$WSD_PREV_SIG" ]; then
      WSD_STABLE=$((WSD_STABLE+1))
    else
      WSD_STABLE=1
    fi
    WSD_PREV_SIG="$WSD_SIG"
    WSD_LAST_XML="$WSD_XML"; WSD_LAST_TITLES="$WSD_TITLES"

    WSD_ATTR_OK=0
    if [ "$WSD_COUNT" -eq 0 ]; then
      WSD_ATTR_OK=1
    elif [ "$WSD_NONSCENE" -gt 0 ]; then
      WSD_ATTR_OK=1
      SESSION_HAS_NONSCENE=1
    elif [ "$WSD_COUNT" -eq 1 ] && [ "$SESSION_HAS_NONSCENE" -eq 1 ]; then
      # A stable single Scene row after earlier real labels may be the real Scene
      # package at the tail of the list. Never use this exception on page 0.
      WSD_ATTR_OK=1
    fi

    if [ "$WSD_ATTR_OK" -eq 1 ] && [ "$WSD_STABLE" -ge "$LABEL_STABLE_DUMPS" ]; then
      STABLE_DETAIL_FILE="$WSD_XML"
      echo "Attribution stable: page=$WSD_PAGE tries=$WSD_TRY rows=$WSD_COUNT nonScene=$WSD_NONSCENE"
      return 0
    fi

    if [ "$WSD_TRY" -eq 1 ]; then
      echo "Waiting for stable Scene app labels: page=$WSD_PAGE rows=$WSD_COUNT nonScene=$WSD_NONSCENE"
    fi
    sleep "$LABEL_WAIT_SECONDS"
    WSD_TRY=$((WSD_TRY+1))
  done

  UNRESOLVED_DETAIL_FILE="$WSD_LAST_XML"
  UNRESOLVED_TITLES_FILE="$WSD_LAST_TITLES"
  return 72
}
...
reopen_history_from_detail() {
  ...
  while [ "$RH_TRY" -lt 6 ]; do
    ...
    if grep -q 'resource-id="com.omarea.vtools:id/action_history"' "$RH_XML"; then
      ...
      sleep 2
      RH_VERIFY="$TMP/reopen-verify-$RH_SEQ-$RH_TRY.xml"; dump "$RH_VERIFY" || return 62
      is_history "$RH_VERIFY" && return 0
    fi
    input swipe 720 900 720 2700 450 >/dev/null 2>&1; sleep 1
    RH_TRY=$((RH_TRY+1))
  done
  return 63
}
...
capture_target_detail() {
  ...
  open_target_detail "$CT_TARGET" "$CT_Y" "$CT_SEQ"; CT_OPEN_RC=$?
  ...
  CT_DP=0; CT_PREV=''; SESSION_HAS_NONSCENE=0
  while [ "$CT_DP" -lt "$MAX_DETAIL_PAGES" ]; do
    wait_stable_detail_page "$CT_SEQ" "$CT_DP"; CT_WAIT_RC=$?
    if [ "$CT_WAIT_RC" -ne 0 ]; then
      if [ "$CT_WAIT_RC" -eq 72 ]; then
        CT_BAD="$INVALIDDIR/detail-$CT_SAFE-$RUN_ID-$CT_SEQ-attribution-unresolved.txt"
        {
          cat "$CT_PART" 2>/dev/null
          printf '\n===== UNRESOLVED PAGE %02d =====\n' "$CT_DP"
          [ -f "$UNRESOLVED_DETAIL_FILE" ] && nodes "$UNRESOLVED_DETAIL_FILE" | grep -E 'avg_power|screen_on_duration|predict_time|itemTitle|itemAvgIO|itemTemperature|itemCounts'
        } > "$CT_BAD"
        rm -f "$CT_PART"
        ensure_manifest
        printf '"%s",invalid_scene_attribution_unresolved,"%s",%s\n' "$CT_TARGET" "$CT_BAD" "$(date '+%Y-%m-%dT%H:%M:%S%z')" >> "$MANIFEST"
        echo "INVALID session=$CT_TARGET reason=attribution_unresolved page=$CT_DP file=$CT_BAD"
      else
        rm -f "$CT_PART"
      fi
      return "$CT_WAIT_RC"
  ...
  # Final guard: a captured session with 2+ app rows cannot be all default Scene.
  raw_attribution_health "$CT_PART"; CT_HEALTH_RC=$?
  if [ "$CT_HEALTH_RC" -ne 0 ]; then
    CT_BAD="$INVALIDDIR/detail-$CT_SAFE-$RUN_ID-$CT_SEQ-final-health.txt"
    mv "$CT_PART" "$CT_BAD"
    ensure_manifest
    printf '"%s",invalid_scene_attribution_final,"%s",%s\n' "$CT_TARGET" "$CT_BAD" "$(date '+%Y-%m-%dT%H:%M:%S%z')" >> "$MANIFEST"
    echo "INVALID session=$CT_TARGET reason=final_attribution_health rc=$CT_HEALTH_RC file=$CT_BAD"
    return 76
  fi
...
echo '# iQOO 12 Pro Scene backlog drain v1.4'
...
while [ "$CAPTURED" -lt "$MAX_CAPTURES" ]; do
  ...
  if [ "$SCAN_RC" -eq 0 ]; then
    NEXT_SEQ=$((CAPTURED+1))
    capture_target_detail "$TARGET" "$TARGET_USED" "$TARGET_Y" "$NEXT_SEQ"; CAP_RC=$?
    [ "$CAP_RC" -eq 0 ] || {
      case "$CAP_RC" in
        72) echo "ERROR attribution_unresolved session=$TARGET";;
        73|74|75|76) echo "ERROR attribution_gate rc=$CAP_RC session=$TARGET";;
  ...
  CUR_N="$TMP/history-scroll-$TOTAL_SWIPES.nodes"; nodes "$CUR" > "$CUR_N"
